...
void matrixHandler(color **img, int NC, int NR,int x, int y,int radius,int stepnum,int step){
    ...
    int SIZE=5;
    int max_arrayr[8],max_arrayg[8],max_arrayb[8],hue=0;
    ...
    if (step != 0){
        ...
        free_color_img(img2);
        free_color_img(img3);
        img_to_ppm_file(img,"562_hw3partb.ppm",NC,NR);/*  write to file */
...
if(argc!=5) {
  printf("\n Usage: ./562hw3partb [center_x] [center_y] [d] [Image file (*.ppm)]\n");
  printf("\n E.g.   ./562hw3partb 200 150 20 [.pgm file]\n");  
  exit(-1); }
